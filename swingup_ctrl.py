...
d1=torch.load('./data_swing_val_vel.pt')
...
model.to(device)
model.load_state_dict(torch.load("./models/swing_up2"))     # Load a previously trained model
...
for i in range(990):
    ...
    ctrl_inp = model.get_ctrl_LQR(z[i],current_state[3:],device)
